...
impl<T> Encrypted<T> {
    pub fn new(data: Vec<u8>) -> Encrypted<T> {
        Encrypted {
            data,
            phantom: PhantomData,
        }
...
pub struct CipherText {
    pub content_type: ContentType,
    pub fragment: Encrypted<StreamCipher>,
}
        ...
        ContentType::read(r).and_then(|content_type| {
            u16::read(r).and_then(|len| {
                r.sub(len as usize).and_then(|mut rd| {
                    ...
                    let fragment = Encrypted::new(data);
...
pub struct StreamCipher {
    content: Vec<u8>, // the same as PlainText.fragment
    mac: Vec<u8>,  // has length of SecurityParams.mac_length 
}
...
struct SessionCommon {
    pub traffic: bool,
    pub peer_eof: bool,
    sendable_plaintext: VecBuffer,
    sendable_tls: VecBuffer,
    received_plaintext: VecBuffer,

    // Buffers incoiming tls cipher test
